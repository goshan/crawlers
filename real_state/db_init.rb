...
db.query(<<~SQL)
  CREATE TABLE IF NOT EXISTS daily_metrics (
    ...
    UNIQUE KEY uq_location_date (location_key, date),
    CONSTRAINT fk_location FOREIGN KEY (location_key) REFERENCES locations(key)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
SQL
puts "Table 'daily_metrics' ready."

stmt = db.prepare("INSERT INTO locations (key, label) VALUES (?, ?) ON DUPLICATE KEY UPDATE label = VALUES(label)")
