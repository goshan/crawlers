...
def render_combined_chart(path, entries, series_map)
  ...
  chart.font = File.expand_path("assets/fonts/NotoSansCJKjp-Regular.otf", __dir__)
  chart.theme = {
    colors: Config::LOCATION_CONFIG.values.map { |conf| conf[:color] },
    marker_color: "#6b7280",
    font_color: "#111827",
  ...
  series_map.each do |key, data|
    ...
    chart.dataxy(Config::LOCATION_CONFIG[key][:label], points, Config::LOCATION_CONFIG[key][:color])
  end
...
series_map = Config::LOCATION_CONFIG.map { |key, _| [key, build_xy_series(entries, key)] }.to_h
